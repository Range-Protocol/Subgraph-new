import {Burn, Mint, Position, Swap, User, UserVaultBalance, Vault, FeeEarned, FeesUpdated} from "../generated/schema";
...
export function handleMinted(event: MintedEvent): void {
    ...
    vault.uniqueKey = vault.uniqueKey.plus(bn(1));
    const mint = new Mint(
        vault.id.toHexString() + "#" + vault.uniqueKey.toHexString()
    );
...
export function handleSwap(event: SwappedEvent): void {
    const vault = Vault.load(event.address)!;
    vault.uniqueKey = vault.uniqueKey.plus(bn(1));
    const swap = new Swap(vault.id.toHexString() + "#" + vault.uniqueKey.toHexString());
